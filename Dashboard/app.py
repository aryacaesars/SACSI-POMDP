...
st.title("SACSI-POMDP · Unified Final Dashboard")
st.caption("Retrospective benchmark 2025 · 9 methods · 10 matched RL seeds · target band 0.22–0.32")
...
with st.sidebar:
    st.header("Final Experiment View")
    mode = st.radio("Mode", ("Single method", "Compare 2–4 methods"), key="view_mode")
    single_method = st.selectbox(
        "Method", method_order, index=0, disabled=mode != "Single method", key="single_method",
    )
    compared_methods = st.multiselect(
        "Methods", method_order,
        default=["Threshold-Based", "SAC + Forecast", "SACSI Full"],
        max_selections=4,
        disabled=mode == "Single method",
        key="compared_methods",
    )
    if mode == "Single method":
        ...
        if len(selected) < 2:
            st.info("Select at least two methods.")
            st.stop()
    has_rl = bool(set(selected) & rl_methods)
    seed_label = st.selectbox(
        "Matched RL seed", ("11", "22", "33", "44", "55", "66", "77", "88", "99", "110"),
        disabled=not has_rl, key="matched_seed",
    )
    seed = int(seed_label)
    st.caption("The seed selector affects RL methods only; deterministic baselines remain one trajectory.")
...
overview_tab, trajectory_tab, ablation_tab, statistics_tab, export_tab = st.tabs((
    "Overview", "Trajectories", "Ablation & Robustness", "Statistics", "Exports",
))

with overview_tab:
    cards = st.columns(4)
    cards[0].metric("Selected methods", len(selected))
    cards[1].metric("Displayed hours", f"{view['timestamp'].nunique():,}")
    cards[2].metric("Best target occupancy", f"{metric_table['time_in_target_pct'].max():.2f}%")
    cards[3].metric("Lowest irrigation", f"{metric_table['total_irrigation_mm'].min():.2f} mm")

    st.subheader("Nine-method final registry")
    registry_columns = [
    ...
    st.dataframe(benchmark_summary[registry_columns], hide_index=True, width="stretch")
    ...
    ranking = px.bar(
        ...
        labels={"time_in_target_pct_mean": "Mean Time in Target (%)", "method": "Method"},
    )
    ...
    st.caption("PNG: hover over a chart and click the camera icon in the Plotly toolbar.")

with trajectory_tab:
    st.subheader("Soil moisture and target band")
    soil = px.line(view, x="timestamp", y="theta", color="controller", labels={"controller": "Method"})
    ...
    soil.update_layout(yaxis_title="Volumetric soil moisture (m³/m³)")
    ...
    with left:
        irrigation = px.line(
            ...
            labels={"irrigation_mm": "Irrigation (mm/h)", "controller": "Method"},
        )
    ...
    with right:
        cumulative = px.line(
            ...
            labels={"cumulative_irrigation_mm": "Cumulative irrigation (mm)", "controller": "Method"},
        )
    ...
    rain = go.Figure()
    rain.add_bar(x=precipitation["timestamp"], y=precipitation["actual_precipitation_mm"], name="Actual")
    rain.add_scatter(
        x=precipitation["timestamp"], y=precipitation["forecast_precipitation_mm"],
        name="SF20 h+1 proxy", mode="lines",
    )
    rain.update_layout(yaxis_title="Precipitation (mm/h)", barmode="overlay")
    st.plotly_chart(rain, width="stretch", config=png_chart_config("rain_actual_forecast"))

    st.subheader("Metrics for displayed period")
    shown = [
    ...
    st.dataframe(metric_table[shown], hide_index=True, width="stretch")

with ablation_tab:
    labels = {
        "context_ablation": "Context ablation",
        "forecast_robustness": "Forecast robustness SF10–SF30",
        "sequence_sensitivity": "Sequence sensitivity k6–k48",
    }
    experiment_label = st.selectbox(
        "Experiment", list(labels.values()), key="ablation_experiment"
    )
    ...
    target_chart = px.bar(
        ...
        labels={"condition": "Condition", "time_in_target_pct_mean": "Mean Time in Target (%)"},
    )
    ...
    st.dataframe(experiment_data, hide_index=True, width="stretch")
    st.caption("All conditions use the same 10 locked seeds; sequence sensitivity is inference-window sensitivity without retraining.")

with statistics_tab:
    st.subheader("Primary endpoint: Time in Target (%)")
    st.dataframe(factorial, hide_index=True, width="stretch")
    st.subheader("Pre-specified paired SACSI comparisons")
    st.dataframe(pairwise, hide_index=True, width="stretch")
    if findings["sacsi_superiority_supported"]:
        st.success("SACSI superiority is supported for all pre-specified comparisons.")
    else:
        st.info("SACSI superiority over all variants is not supported. Technical validity remains separate from superiority.")
    st.caption("Deterministic baselines are trajectory references and are not assigned fake stochastic seeds.")

with export_tab:
    st.subheader("Export selected experiment view")
    metadata = {
    ...
    columns[0].download_button("Download CSV", csv_bytes, f"{filename}.csv", "text/csv")
    columns[1].download_button(
        "Download XLSX", xlsx_bytes, f"{filename}.xlsx",
        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    )
    columns[2].download_button("Download JSON", json_bytes, f"{filename}.json", "application/json")
    columns[3].download_button("Download ZIP", zip_bytes, f"{filename}.zip", "application/zip")
    st.info("PNG export is available from the camera icon on every chart toolbar.")
